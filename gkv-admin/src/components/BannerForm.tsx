...
import {Form, InputNumber, Button, Modal, Upload, message, Flex} from 'antd'
...
const BannerForm: React.FC<BannerFormProps> = ({visible, onClose, onSuccess, banner}) => {
  ...
  const handleSubmit = async (values: Omit<Banner, 'id'>) => {
    ...
    try {
      ...
      if (response) {
        message.success(`Banner ${isEditing ? 'updated' : 'added'} successfully!`)
        onSuccess(response)
        onClose()
        form.resetFields()
        setFile(null)
