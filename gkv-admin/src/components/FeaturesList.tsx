...
import { Table, Input, Select, Pagination, Switch, message, Card } from "antd";
...
const FeaturesList: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "typeId",
      render: (val: number) =>
        categories.find((c) => Number(c.id) === Number(val))?.name || "N/A",
    },
    {
      title: "Featured",
      ...
      render: (val: boolean, record: any) => (
        <Switch
          checked={!!val}
          onChange={(checked) => toggleFeatured(record.id, checked)}
        />
      ),
    },
  ];
